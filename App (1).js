...
class App extends Component {
  ...
  timeDecrement = () => {
    ...
    if (isTimer <= 0) {
      ...
      this.setState(prevState => ({isTimer: prevState.isTimer - 1}))
    }
  }

  timeRunning = () => {
    this.setIntervalId = setInterval(this.timeDecrement, 1000)
  }

  generateRandomImg = () => {
    const random = Math.ceil(Math.random() * imagesList.length - 1)
    const gen = imagesList[random]

    this.setState({displayImg: gen})
  }
  ...
  render() {
    const {score, isTimer, displayImg, activeTabId, showResults} = this.state

    const filterTabImage = this.filterImagesList(activeTabId)
    ...
    return (
      <div>
        <nav className="navbar-container">
          ...
          <ul className="nav-last-container">
            <li>
            ...
            <li>
              <div className="clock-container">
                <img
                  ...
                  alt="timer"
                />
                <p className="time">{isTimer} sec</p>
              </div>
            </li>
          </ul>
        </nav>
        {showResults ? (
          <div className="bg-container-two">
            <ResultItem score={score} onResetClick={this.onResetClick} />
          </div>
        ) : (
